...
- [12 — Research: Codex CLI & Antigravity CLI headless mode](issues/12-research-codex-agy-headless.md) — Background research only; does not change the fixed two-CLI pipeline. Codex (`codex exec`) fits the existing machinery: stdin prompts, an OS-enforced `-s read-only` sandbox, JSONL events, `-C/--cd`, and no per-directory setup. Antigravity (`agy`) also takes stdin prompts (only when `--print` is omitted) and does stream text via `text_delta`, but two obstacles remain: no working-directory flag plus a `trustedWorkspaces` gate that `--add-dir` doesn't bypass — versus our per-run ephemeral worktrees — and a silent-failure mode returning exit 0 with `status: "SUCCESS"` and an empty response. Neither reports cost; both report token counts, with field sets matching neither each other nor Claude Code. Includes a cross-review (§10) where each CLI reviewed the doc's claims about itself and every disputed claim was re-probed, correcting four defects.
