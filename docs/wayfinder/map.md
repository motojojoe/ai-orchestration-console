...
- [12 — Research: Codex CLI & Antigravity CLI headless mode](issues/12-research-codex-agy-headless.md) — Background research only; does not change the fixed two-CLI pipeline. Codex (`codex exec`) fits the existing machinery: stdin prompts, an OS-enforced `-s read-only` sandbox, JSONL events, `-C/--cd`. Antigravity (`agy --print`) does not fit cleanly: argv-only prompts (bounded by `ARG_MAX`, colliding with our deliberate stdin design), no working-directory flag plus a `trustedWorkspaces` gate, and a silent-failure mode that returns exit 0 with `status: "SUCCESS"` and an empty response. Neither reports cost; both report token counts, with field sets that match neither each other nor Claude Code.
